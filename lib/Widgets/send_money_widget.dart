...
class SendMoneyFormWidget1 extends StatefulWidget {
  final String email;
  const SendMoneyFormWidget1({super.key, required this.email});

  @override
  State<SendMoneyFormWidget1> createState() => _SendMoneyFormWidget1State();
}

class _SendMoneyFormWidget1State extends State<SendMoneyFormWidget1> {
  final GlobalKey<FormState> _formKey = GlobalKey();
  Map<String, String> _sendMoneyData = {
    "email": "",
    "amount": "",
  ...
  Future<void> fetchReciverUser(String email) async {
    try {
      await UserProvider.fetchUserByEmail(email).then((value) {
        ReceiverData.ReceiverEmail = email;
        ReceiverData.Transferamount = _amountController.text;
        ReceiverData.Transferremarks = _remarksController.text;
        ReceiverData.TransferPurpose = purpose;
        print(purpose);
  ...
  Future<void> _submit() async {
    ...
    await fetchReciverUser(widget.email);
    _formKey.currentState!.save();
  }
